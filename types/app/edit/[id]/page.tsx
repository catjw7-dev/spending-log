...
import { getTransactions, updateTransaction } from "@/lib/storage";
...
export default function EditPage() {
  ...
  const displayAmount = amount
    ? new Intl.NumberFormat("ko-KR").format(parseInt(amount.replace(/[^0-9]/g, "")))
    : "";
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <div className="flex items-center px-4 pt-14 pb-4">
        <button onClick={() => router.back()} className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-toss-bg transition-colors mr-2">
          <ChevronLeft size={22} className="text-toss-text" />
        </button>
        <h1 className="text-[17px] font-semibold text-toss-text">내역 수정</h1>
      </div>
        ...
        <div className="flex bg-toss-bg rounded-2xl p-1">
          {(["expense", "income"] as TransactionType[]).map((t) => (
            <button key={t} onClick={() => handleTypeChange(t)}
              className={`flex-1 py-2.5 rounded-xl text-[14px] font-semibold transition-all ${type === t ? (t === "expense" ? "bg-white text-toss-red shadow-card" : "bg-white text-toss-green shadow-card") : "text-toss-text-4"}`}>
              {t === "expense" ? "지출" : "수입"}
            </button>
          ))}
        </div>
      ...
      <div className="px-5 mb-6">
        ...
        <input type="text" placeholder="어디에 썼나요?" value={description}
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setCategory(cat.id)}
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-full text-[13px] font-medium transition-all border ${category === cat.id ? (type === "expense" ? "bg-toss-red text-white border-toss-red" : "bg-toss-green text-white border-toss-green") : "bg-white text-toss-text-3 border-toss-border"}`}>
      ...
      <div className="px-5 mt-auto pb-10">
        <button onClick={handleSubmit} disabled={!isValid || saving}
          className={`w-full py-4 rounded-2xl text-[16px] font-semibold transition-all flex items-center justify-center gap-2 ${isValid && !saving ? "bg-toss-blue text-white active:scale-[0.98]" : "bg-toss-bg text-toss-text-5 cursor-not-allowed"}`}>
