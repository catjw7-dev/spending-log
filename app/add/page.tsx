...
export default function AddPage() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const tx: Transaction = { id: uuidv4(), type, amount: parseInt(amount), category, description, date, createdAt: Date.now() };
  ...
  return (
    <div className="min-h-screen max-w-full overflow-x-hidden bg-white dark:bg-toss-card-dark flex flex-col">
      <div className="flex items-center px-4 pt-14 pb-4">
        ...
        <h1 className="text-[17px] font-semibold text-toss-text dark:text-white">내역 추가</h1>
      </div>
      ...
      <div className="px-5 mt-auto pb-10">
        <button onClick={handleSubmit} disabled={!isValid || saving}
          ...
          {saving ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><Check size={18} strokeWidth={2.5}/>저장하기</>}
        </button>
      </div>
    </div>
  );
