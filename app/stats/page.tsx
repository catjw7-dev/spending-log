...
export default function StatsPage() {
  ...
  const CustomTooltip = ({active,payload}: any) => {
    if (active && payload?.length) return (
      <div className="bg-white dark:bg-toss-card-dark rounded-xl px-3 py-2 shadow-card-hover border border-toss-border dark:border-toss-border-dark">
        {payload.map((p: any) => <p key={p.name} style={{color:p.color}} className="text-[12px] font-semibold">{p.name==="income"?"수입":"지출"}: {formatKRW(p.value)}</p>)}
      </div>
    );
    return null;
  };

  if (loading) return <div className="flex items-center justify-center min-h-screen bg-toss-bg dark:bg-[#0d1117]"><div className="w-6 h-6 border-2 border-toss-blue border-t-transparent rounded-full animate-spin" /></div>;
      ...
      <div className="px-4 pt-4 space-y-4">
        <div className="bg-white dark:bg-toss-card-dark rounded-2xl p-5 shadow-card">
          ...
          <div className="flex justify-between">
            <div><p className="text-[12px] text-toss-text-4">수입</p><p className="text-[22px] font-bold text-toss-green mt-1">+{formatKRW(income)}</p></div>
            <div className="text-right"><p className="text-[12px] text-toss-text-4">지출</p><p className="text-[22px] font-bold text-toss-red mt-1">-{formatKRW(expense)}</p></div>
          </div>
          {income > 0 && (
            <div className="mt-4">
              <div className="h-2 bg-toss-bg dark:bg-toss-bg-dark rounded-full overflow-hidden">
        ...
        {catData.length > 0 ? (
            ...
            <div className="space-y-2.5 mt-2">
              {catData.map((cat,i)=>(
                <div key={cat.name} className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-toss-bg dark:bg-toss-bg-dark flex items-center justify-center text-[14px] flex-shrink-0">{getCategoryEmoji(cat.name)}</div>
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      <span className="text-[13px] font-medium text-toss-text dark:text-white">{cat.name}</span>
                      <span className="text-[13px] font-semibold text-toss-text dark:text-white">{formatKRW(cat.value)}</span>
                    </div>
          ...
          <div className="bg-white dark:bg-toss-card-dark rounded-2xl p-10 shadow-card text-center">
